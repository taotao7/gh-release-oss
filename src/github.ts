...
export const upload = async (
  ...
  try {
    await client.multipartUpload(name, body, {
      progress: function (p) {
        //progress is generator
        console.log(`${name} progress`, p * 100);
      },
    });
    client.putACL(name, "public-read").then((d) => {
      console.log("putACL", d);
    });
  } catch (error) {
    console.log(`⚠️ Upload failed: ${error}`);
  }
